...
function ShoppingListScreen() {
  ...
  // Count items that are in cart
  const inCartCount = shoppingList.filter(item => item.isBought).length;

  // Sort the shopping list to move checked items to the end
  const sortedShoppingList = useMemo(() => {
    return [...shoppingList].sort((a, b) => {
      if (a.isBought && !b.isBought) return 1; // Move checked items down
      if (!a.isBought && b.isBought) return -1; // Move unchecked items up
