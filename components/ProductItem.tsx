...
function ProductItem({ id, name, quantity, unit }: ProductItemProps) {
  ...
  // Count how many of this product are in the shopping list
  const countInList = shoppingList.filter(item => item.id === id).length;

  // Reset selected quantity if product quantity changes
  useEffect(() => {
  ...
  function handleAddToShoppingList() {
    ...
    // Show the "Added" indicator temporarily
    setAdded(true);
    setTimeout(() => {
      setAdded(false);
    }, 1000);
  ...
  function handleIncrement() {
    // Different increment values based on unit type
    let increment = 1;
  ...
  function handleDecrement() {
    // Different decrement values based on unit type
    let decrement = 1;
