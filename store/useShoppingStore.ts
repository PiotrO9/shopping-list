  ...
  toggleBought: (id: string, index?: number) => {
    set((state) => ({
      shoppingList: state.shoppingList.map((item, idx) => {
        // If an index is provided, only toggle that specific item instance
        if (index !== undefined) {
          return idx === index ? { ...item, isBought: !item.isBought } : item;
        }
        // Otherwise toggle all items with the matching id (backward compatibility)
...
// Sample product data with a comprehensive grocery list
